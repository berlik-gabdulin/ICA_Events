...
const HomePageBlock = async (req: NextApiRequest, res: NextApiResponse<IPageBlock | IAPIError>) => {
  const { block_name } = req.query;

  if (req.method === 'GET') {
    try {
      const [rows] = await pool.execute<RowDataPacket[]>(
        'SELECT * FROM page_home WHERE block_name = ?',
        [block_name]
      );
      if (rows.length) {
    ...
    try {
      await pool.execute('UPDATE page_home SET content = ? WHERE block_name = ?', [
        content,
        block_name,
      ]);
